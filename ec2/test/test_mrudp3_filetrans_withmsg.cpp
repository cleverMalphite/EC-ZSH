...
//MRUDP数据服务b回调函数
void mrudp_dataserver_b_callback(
            DWORD dTermId,
            const std::shared_ptr<BYTE> &recv_data,
            DWORD dRecvDataByteLength);
...
int main(int argc, char *argv[]) {
    ...
    if(starg.role==1){
      ...
      /*****************发送数据服务a/b的数据*************************/
      //向目标发送数据服务的数据 (server type = msg_a)
      char strLocalTime[21]; memset(strLocalTime, 0, sizeof(strLocalTime));
      LocalTime(strLocalTime, "yyyymmddhh24miss");
      msg::msg_a ma ;
      ma.set_server_type("msg_a"); ma.set_time(strLocalTime); ma.set_id(0001);
      int ma_size = ma.ByteSizeLong();
      std::shared_ptr<BYTE> pBuffer_a(new BYTE[ma_size]);
      if(ma.SerializeToArray(pBuffer_a.get(), ma_size)==false){logfile.Write("ma序列化失败!\n"); return -1;}
      while( SendDataBytesToTermByMRUDP(starg.remotetid, pBuffer_a, ma_size)==false){sleep(1);}
      logfile.Write("发送ma类型数据成功!\n");

      //向目标发送数据服务的数据 (server type = msg_b)
      msg::msg_b mb ;
      mb.set_server_type("msg_b"); mb.set_time(strLocalTime); mb.set_id(0002);
      int mb_size = mb.ByteSizeLong();
      std::shared_ptr<BYTE> pBuffer_b(new BYTE[mb_size]);
      if(mb.SerializeToArray(pBuffer_b.get(), mb_size)==false){logfile.Write("mb序列化失败!\n"); return -1;}
      while( SendDataBytesToTermByMRUDP(starg.remotetid, pBuffer_b, mb_size)==false){sleep(1);}
      logfile.Write("发送mb类型数据成功!\n");
    ...
    //接收方
      ...
      //注册服务数据回调函数——数据服务名称是"msg_b"
      if(register_mrudp_dataserver_callback("msg_b", mrudp_dataserver_b_callback)==false)
      {
        logfile.Write("接收方的mrudp数据服务b的回调-注册失败！\n");
        return -1;
      }
            ...
            DWORD dRecvDataByteLength)
{
  ...
  if(ma.ParseFromArray(recv_data.get(), dRecvDataByteLength)==false) {
    ...
    logfile.Write("[mrudp_dataserver a]::ma.ParseFromArray failed\n");
  }else{
    ...
    logfile.Write("[mrudp_dataserver a]::recv data: =%s:%s:%d=\n",
           ma.server_type().c_str(), ma.time().c_str(), ma.id());
  }
}
...
{
  msg::msg_b mb;
  logfile.Write("mrudp_dataserver_b_callback get data\n");
  if(mb.ParseFromArray(recv_data.get(), dRecvDataByteLength)==false) {
    printf("[mrudp_dataserver b]::ma.ParseFromArray failed\n");
    logfile.Write("[mrudp_dataserver b]::ma.ParseFromArray failed\n");
  }else{
    printf("[mrudp_dataserver b]::ma.ParseFromArray succeed!\n");
    printf("[mrudp_dataserver b]::recv data: =%s:%s:%d=\n", 
           mb.server_type().c_str(), mb.time().c_str(), mb.id());
    logfile.Write("[mrudp_dataserver b]::recv data: =%s:%s:%d=\n", 
           mb.server_type().c_str(), mb.time().c_str(), mb.id());
  }
}
...
{
  printf("\n");
  printf("Using:./test_mrudp3_filetrans_withmsg logfilename xmlbuffer\n\n");

  printf("Sample0.1 [发送方]:\n"\
		"./test_mrudp3_filetrans_withmsg ../log/testmrudp3_01.log \""\
  ...
  printf("Sample0.2 [接收方]:\n"\
		"./test_mrudp3_filetrans_withmsg ../log/testmrudp3_02.log \""\
