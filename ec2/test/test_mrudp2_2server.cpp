...
int main(int argc, char *argv[]) {
    ...
    if(starg.role==1){
      ...
      if(mb.SerializeToArray(pBuffer_b.get(), mb_size)==false)
      {
        logfile.Write("mb序列化失败!\n");
      }

      //向目标终端发送文件
      //没发送成功，可能是双端还未连接好
      while( SendDataBytesToTermByMRUDP(102, pBuffer_b, mb_size) == false )
      {sleep(1);};
      logfile.Write("发送mb类型的数据成功！\n");
...
{
  printf("\n");
  printf("Using:./test_mrudp2_2server logfilename xmlbuffer\n\n");

  printf("Sample0.1 [非DTU，发送方]:\n"\
		"./test_mrudp2_2server ../log/testmrudp_madata01.log \""\
  ...
  printf("Sample0.2 [非DTU，接收方]:\n"\
		"./test_mrudp2_2server ../log/testmrudp_madata02.log \""\
