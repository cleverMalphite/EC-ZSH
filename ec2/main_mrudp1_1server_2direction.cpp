...
int main(int argc, char *argv[]) {
    ...
    if(starg.role==1){
      ...
      infohub_instance->value_set("EC", "main_mrudp1_1server", "sender");
      ...
      if(register_mrudp_dataserver_callback("msg_a_2", mrudp_dataserver_a_callback)==false)
      {
    ...
    //接收方
      ...
      infohub_instance->value_set("EC", "main_mrudp1_1server", "receiver");
      ...
      int generate_seed = 0;
        ...
        if(ma.SerializeToArray(pBuffer.get(), msg_size)==false)
        {
          logfile.Write("ma_2序列化失败!\n");
        }

        //没发送成功，可能是双端还未连接好
        while( SendDataBytesToTermByMRUDP(101, pBuffer, msg_size) == false )
        {sleep(1);};
        logfile.Write("发送ma_2类型的数据成功！\n");
			}
              ...
              DWORD dRecvDataByteLength)
{
  ...
  if(ma.ParseFromArray(recv_data.get(), dRecvDataByteLength)==false) {
    ...
    printf("[mrudp_dataserver a]::recv data: =%s:%s:%d=\n\n", ma.server_type().c_str(), ma.time().c_str(), ma.id());
  	...
  	logfile.Write("[mrudp_dataserver a]::ma.ParseFromArray succeed!\n");
...
void _help(char *argv[])
{
  ...
  printf("Sample0.1 [非DTU，发送方]:\n"\
		"./main_mrudp1_1server_2direction ../log/testmrudp_madata_2direction_01.log \""\
  ...
  printf("Sample0.2 [非DTU，接收方]:\n"\
		"./main_mrudp1_1server_2direction ../log/testmrudp_madata_2direction_02.log \""\
