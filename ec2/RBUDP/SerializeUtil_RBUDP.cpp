...
namespace RBUDP
{
    ...
    const static DWORD RBUDP_WITHOUT_RELIABLE_HEADER_LENGTH = 5;	//标志位加字节数
    /**
 ...
 */
    std::shared_ptr<BYTE> RBUDP_Encode_Bytes_Without_Reliable(std::shared_ptr<BYTE> bytes, DWORD old_len, DWORD &new_len) {
        new_len = old_len + RBUDP_WITHOUT_RELIABLE_HEADER_LENGTH;
    ...
    std::shared_ptr<BYTE>
    RBUDP_Decode_Data_Without_Reliable(std::shared_ptr<BYTE> bytes, DWORD &nLength) {
        //获取有效字节长度
