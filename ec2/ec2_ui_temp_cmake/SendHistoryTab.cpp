...
SendHistoryTab::SendHistoryTab(QWidget *parent)
    : QWidget(parent)
{
    setupUI();
}

void SendHistoryTab::setupUI()
{
    ...
    m_table->setHorizontalHeaderLabels({
        QStringLiteral("任务号"),
        QStringLiteral("接收端号 (TID)"),
        QStringLiteral("文件名"),
        QStringLiteral("状态"),
        QStringLiteral("平均发送速率"),
        QStringLiteral("发送时间")
    });
...
int SendHistoryTab::findRowByTaskId(unsigned int taskId) const
{
...
QString SendHistoryTab::formatRate(float kbps)
{
    if (kbps >= 1024.0f) {
...
void SendHistoryTab::applyStatusColor(QTableWidgetItem *statusItem, const QString &status)
{
    if (!statusItem) {
        return;
    }
    if (status == QStringLiteral("发送成功")) {
        statusItem->setForeground(QColor("#28a060"));
    } else if (status == QStringLiteral("发送失败")) {
        statusItem->setForeground(QColor("#dc3545"));
    } else {
        statusItem->setForeground(QColor("#f5a623"));
    }
}

void SendHistoryTab::addSendRecord(unsigned int taskId,
                                   ...
                                   const QString &sendTime)
{
    ...
    m_table->setItem(row, 1, new QTableWidgetItem(receiverTid == 0 ? QStringLiteral("--") : QString::number(receiverTid)));
    ...
    m_table->setItem(row, 5, new QTableWidgetItem(sendTime));
}

void SendHistoryTab::upsertSendProgress(unsigned int taskId,
                                        unsigned int receiverTid,
                                        const QString &fileName,
                                        const QString &status,
                                        float rateKbps,
                                        const QString &sendTime)
{
    ...
    if (row < 0) {
        addSendRecord(taskId, receiverTid, fileName, status, rateKbps, sendTime);
        return;
    }

    if (auto *tidItem = m_table->item(row, 1)) {
        if (receiverTid != 0) {
            tidItem->setText(QString::number(receiverTid));
        }
    }
    ...
    if (auto *timeItem = m_table->item(row, 5)) {
        timeItem->setText(sendTime);
    }
}

void SendHistoryTab::updateTaskStatus(unsigned int taskId, const QString &newStatus)
{
    const int row = findRowByTaskId(taskId);
    if (row < 0) {
        return;
...
void SendHistoryTab::clearRecords()
{
