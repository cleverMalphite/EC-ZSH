...
#include <thread>    // 引入C++11线程支持
#include <atomic>    // 引入原子操作，确保线程安全
//#include <unistd.h>  // 提供sleep功能
...
// 文件发送线程函数
void FileSendThread() {
    // 等待 startAutoSend 置为 true 后开始文件传输
    while (true) {
        if (startAutoSendFlag.load()) {
            logfile.Write("收到StartAutoSend信号，发送方开始批量传输!\n");
            while (startAutoSendFlag.load()) {  // 继续发送文件，直到 startAutoSend 被重置
                _ectransfiles();
                sleep(starg.timetvl);  // 使用 starg.timetvl 控制传输间隔
            }
            logfile.Write("批量传输结束\n");
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(500)); // 休眠，避免空转
    }
}
