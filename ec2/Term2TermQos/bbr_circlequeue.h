...
enum QOS_CircularDataQueueIndexStatus
{
    /* 分以下几种情况：
    * 1. 队列首索引比队列尾索引小；
    * 2. 队列首索引比队列尾索引大；
...
{
public:
    CircularDataQueue(DWORD capacity/*DWORD dSelfTermId, DWORD dRemoteTermId*/)
    {
...
private:
    ...
    std::vector<shared_ptr<packet_feedback> > m_queue_array;	//用来存储循环队列数据的数组
    ...
    //void DealSingleReceiveData(shared_ptr<packet_feedback> data);	//处理暂存数据接收队列中的单个元素
public:
    /**
     * 向数组中添加一个元素，如果成功添加则返回true，否则返回false
     */
    bool AddItem(shared_ptr<packet_feedback> p_item);
    bool SetHeadIndex(DWORD dHeaderIndex);
    bool Qos_DealSingleSendData(const shared_ptr<packet_feedback>& p_item);
    DWORD GetHeadIndex() { return m_queue_head; }
    DWORD GetQueueCapacity() { return QOS_CYCLEFILE_CAPACITY; }
    shared_ptr<packet_feedback> GetItemByIndex(DWORD dIndex);
    DWORD GetTailIndex() { return m_queue_tail; }
    bool IsIndexInValid(DWORD dIndex);
    /**
...
private:
     ...
     */
    QOS_CircularDataQueueIndexStatus GetStatusValidQueue();
};
