...
namespace BigDataTransfer {
    std::shared_ptr<BYTE> FileSendTaskCancel::Encode(DWORD &dDataLength) {
    ...
    bool FileSendTaskCancel::Decode(std::shared_ptr<BYTE> pBuffer, DWORD dLength) {
        BYTE *pTemp = pBuffer.get();
    ...
    bool FileSendTaskCancel::IsEncodeInstanceOf(std::shared_ptr<BYTE> pData, DWORD dDataLength) {
